...
import { getLocalFilePathsSync, getLocalFilesStats, isEmpty, regenerateDatabase } from './utils';
  ...
  if (isEmpty(sourcePath)) {
    ...
    for (const remoteFile of remoteFiles) {
      if (remoteFile.isDownloadable) {
        ...
        const remoteFileWithContent = await client.getRemoteFile(remoteFile.pathDisplay || '');
    ...
    for (const remoteFile of remoteFiles) {
      if (remoteFile.isDownloadable) {
        ...
        if (localFileIsSynched) {
          ...
          await saveFileToLocal(await client.getRemoteFile(remoteFile.pathDisplay || ''), localFilePath);
    ...
    for (const fileStats of localFilesStats) {
      const remoteFile = remoteFiles.find((remoteFile) => remoteFile.pathDisplay === fileStats.path.replace(sourcePath, '/'));
      if (remoteFile) {
        const remoteFileIsSynched = remoteFile.serverModified > fileStats.modified;
        if (remoteFileIsSynched) {
          console.info(`Remote file: ${remoteFile.pathDisplay} is already synched`);
        } else {
          setTimeout(() => {
            //uploadFile(dbx, path.srcPath, path.dstPath)
            client.uploadLocalFile(remoteFile.pathDisplay, fileStats.path);
            console.log(`Remote file: ${remoteFile.pathDisplay} was successfully synched`);
          }, 2000);
        }
      }
    }
